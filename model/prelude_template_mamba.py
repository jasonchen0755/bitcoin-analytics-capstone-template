...
# --------------------------------------------------------------------------------- #
#           Copied from template.prelude_template.py                                #
#   Because Mamba output signals in a rolling manner, some functions were revised.  #
#   1. compute_cycle_spd:                                                           #
#       (1) signal_dict is passed to this function in stead of feature_df.          #
#       (2) the function will try load signal from data/dca/mamba_signals.json.     #
#       (3) signals are {window_start: ndarray}, not in dataframe format.           #
#   2. backtest_dynamic_dca:                                                        #
#       input signal_dict rather than feature_df                                    #
#   3. check_strategy_submission_ready:                                             #
#       (1) first, load precomputed signals.                                        #
#       (2) recomputing signals need mamba installation and cuda.                   #
#       (3) clip variable 'probe' maximum to be '2024-12-31',the lagest signal key. #
# --------------------------------------------------------------------------------- # 
...
def compute_cycle_spd(
    ...
    signal_dict: dict | None = None,#change from pd.DataFrame to dict by mamba model design, not None
    ...
    if signal_dict is None:
        import os, json
        if os.path.exists('data/mamba_signal.json'):
            with open ('data/dca/mamba_signals.json', 'r') as f:
                signal_dict = json.load(f)
        else:
            from model.mamba_backtest import compute_signal
            signal_dict = compute_signal()

    full_signal = {k:np.array(v) for k,v in signal_dict.items()}
    ...
    for window_start in start_dates:
        ...
        if len(price_slice) > 365:
            price_slice = price_slice.iloc[:365] 
        if price_slice.empty:
            continue

        # Compute weights using strategy_function
        # change -> weight slice are precomputed by mamba model
        weight_slice = full_signal[window_start.strftime('%Y-%m-%d')]
        # window_feat = full_feat.loc[window_start.strftime('%Y-%m-%d'): window_end]
        ...
        if len(weight_slice) != len(inv_price):
            print(f'weight:{weight_slice.shape}, inv_price: {inv_price.shape}, start: {window_start}, end: {window_end}')
...
def backtest_dynamic_dca(
    ...
    signal_dict: dict | None = None,#change from pd.DataFrame to dict by mamba model design
    *,
...
def check_strategy_submission_ready(dataframe: pd.DataFrame, # btc_df
    ...
    # change: full weights is a dictionary with keys are firste day of every window
    import os, json
    if os.path.exists('data/dca/mamba_signals.json'):
        ...
        from model.mamba_backtest import compute_signal
        full_weights = compute_signal()
    full_weights = {k:np.array(v) for k,v in full_weights.items()}
